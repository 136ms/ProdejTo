<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Přidejte přídavné bezpečí svému účtu použitím dvoufázového ověření.') }}
    </x-slot>
        ...
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Dokončete dvoufázové ověření.') }}
                @else
                    {{ __('Dvoufázové ověření zapnuto.') }}
                @endif
            @else
                {{ __('Nemáte zapnuto dvoufázové ověření.') }}
            @endif
        </h3>
                ...
                @if ($showingRecoveryCodes)
                    <x-jet-confirms-password wire:then="regenerateRecoveryCodes">
                        <x-jet-secondary-button class="mr-3">
                            {{ __('Znovu vygenerovat zotavovací kódy') }}
